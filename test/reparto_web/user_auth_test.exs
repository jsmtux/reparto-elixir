defmodule RepartoWeb.UserAuthTest do
  use RepartoWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Reparto.Accounts
  alias Reparto.Accounts.Scope
  alias RepartoWeb.UserAuth

  import Reparto.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RepartoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RepartoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RepartoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
